...
let cf0 = Credit("Crédit en raison de l'âge",
    (x, params) => {
        cp = params["FED_AGE"]
...
cf0.type = `Fédéral`;
cf0.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-impot-age/";
cf0.type_credit = "Non remboursable";
let fed = [
    cf0
];

let cp4 = Credit("Crédit d'impôt pour le soutien des aînés",
    (x) => {
        
        return x.age >= CREDIT_PARAMETERS["PROV_SOUTIEN_AINES"].MIN_AGE && !CREDIT_PARAMETERS["PROV_SOUTIEN_AINES"].HAS_PARTNER;
    },
    203, CREDIT_PARAMETERS["PROV_SOUTIEN_AINES"].MAX_REV, 0.05, 0,
    null
);
cp4["reference"] = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-dimpot-pour-le-soutien-des-aines/";
cp4.type_credit = "Remboursable";
cp4.category = "Mesures relatives aux aînés";

let cp5 = Credit("Crédit d'impôt pour le soutien des aînés",
    (x) => {
        return x.age >=  CREDIT_PARAMETERS["PROV_SOUTIEN_AINES_2"].MIN_AGE && revenu_familial(x) < CREDIT_PARAMETERS["PROV_SOUTIEN_AINES_2"].MAX_REV_FAM && (x.hasPartner && x.partner.age < CREDIT_PARAMETERS["PROV_SOUTIEN_AINES_2"].MAX_PARTNER_AGE);
    },
   
);  
cp5.type_credit = "Remboursable";
cp5.category = "Mesures relatives aux aînés";
cp5["note"] = `Le crédit d’impôt remboursable pour le soutien des aînés est partageable entre les conjoints selon la proportion dont ils auront convenu au moyen d’un formulaire prescrit TP-1029.SA
...
cp5["reference"] = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-dimpot-pour-le-soutien-des-aines/";

let cp6 = Credit("Crédit d'impôt pour le soutien des aînés",
    (x) => {
        return x.age >= CREDIT_PARAMETERS["PROV_SOUTIEN_AINES_3"].MIN_AGE && (x.hasPartner && x.partner.age >= CREDIT_PARAMETERS["PROV_SOUTIEN_AINES_3"].MIN_PARTNER_AGE) && revenu_familial(x) < CREDIT_PARAMETERS["PROV_SOUTIEN_AINES_3"].MAX_REV_FAM;
    }
);
cp6["note"] = `Le crédit d’impôt remboursable pour le soutien des aînés est partageable entre les conjoints selon la proportion dont ils auront convenu au moyen d’un formulaire prescrit TP-1029.SA
« Crédit d’impôt pour soutien aux aînés ». À défaut d’entente entre les particuliers, le ministre du Revenu déterminera le montant du crédit d’impôt que chacun pourra demander.`;
cp6["reference"] = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-dimpot-pour-le-soutien-des-aines/";
cp6.type_credit = "Remboursable";
cp6.category = "Mesures relatives aux aînés";


let cp7 = Credit("Crédit d'impôt pour activités des aînés",
    (x) => {
        return x.age >= CREDIT_PARAMETERS["PROV_ACT_AINES"].MIN_AGE && (revenu_familial(x) < CREDIT_PARAMETERS["PROV_ACT_AINES"].MAX_REV_FAM) && x.frais_inscription_adhesion == "true";
    }
);
cp7["reference"] = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-impot-activites-aines/";
cp7.type_credit = "Remboursable";
cp7.category = "Mesures relatives aux aînés";

let cp9 = Credit("Fractionnement des revenus de retraite entre conjoints",
    (x) => {
        return x.hasPartner && x.age >= CREDIT_PARAMETERS["PROV_FRAC_REV_RETRAITE"].MIN_AGE && x.revenue_type.retraite;
    }
);
...
`;
cp9.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/fractionnement-revenus-pension-et-fractionnement-revenus-retraite-conjoints/";


let cp11 = Credit("Crédit d'impôt pour activités des enfants", //REMBOURSABLE
    (x) => {
        return x.frais_engages.activite_des_enfants && revenu_familial(x) <= CREDIT_PARAMETERS["PROV_ACTIVITE_ENFANT"].MAX_REV_FAM;
    }
);
...
l’enfant.
`;
cp11.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-impot-activites-enfants/";
cp11.type_credit = "Remboursable";

let cp13 = Credit("Crédit d'impôt pour activités des enfants",
    (x) => {

        let val = false;
        if (x.hasKids) {
            for (let i = 0; i < x.kidAmount; i++) {
                val = val || (x.kids[i].frais_engages.activite_enfant && x.kids[i].age < 16 && x.kids[i].age > 5)
            }
        }
        return val && revenu_familial(x) < CREDIT_PARAMETERS["PROV_ACTIVITE_ENFANT"].MAX_REV_FAM;
    }
);
cp13.category = "Mesures relatives aux enfants";
cp13.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-impot-activites-enfants/";
cp13.type_credit = "Remboursable";

let cp15 = Credit("Déduction pour travailleurs",
    (x) => {
...
cp15.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/deduction-travailleurs/";


let cp21 = Credit("Montant pour autres personnes à charge", //Non remboursable 
    (x) => {
        ...
        for (let i = 0; i < x.kidAmount; i++) {
            if (x.kids[i].age >= 18
                ...
                && x.kids[i].revenue <  CREDIT_PARAMETERS["PROV_PERSONNE_CHARGE"].MAX_KID_REV
            ) {
                return true;
            }
        }
...
cp21.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/montant-personnes-a-charge/";

let cp20 = Credit("Déduction pour frais de déménagement",
    (x) => {
...
cp20.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/frais-demenagement/";

let cp25 = Credit("Montant pour enfant aux études postsecondaires", //NON REMBOURSABLE, POUR DEUX SESSIONS (TODO)
    (x) => {
        return hasKid(x, kid => {
            return kid.age < 18 && kid.status_etudiant == "temps_plein" && kid.revenue <  CREDIT_PARAMETERS["PROV_MONTANT_POSTSEC"].MAX_KID_REV
        });
    }
);
...
cp25.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/montants-etudes-postsecondaires/"

let cp26 = Credit("Montant transféré par un enfant majeur aux études postsecondaires", //NON REMBOURSABLE, POUR DEUX SESSIONS (TODO)
    (x) => {
        return hasKid(x, kid => {
            return kid.age >= 18 && kid.status_etudiant == "temps_plein" && kid.revenue < CREDIT_PARAMETERS["PROV_MONTANT_POSTSEC_1"].MAX_KID_REV
        });
    }
);
...
cet effet, les bourses d’études reçues par l’étudiant durant l’année ne sont pas prises en considération.
`;
cp26.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/montants-etudes-postsecondaires/";

let cp27 = Credit("Crédit d'impôt pour la prolongation de carrière",
    (x) => {
        return x.revenue > CREDIT_PARAMETERS["PROV_PROL_CARRIERE"].MIN_REV && x.age >=  CREDIT_PARAMETERS["PROV_PROL_CARRIERE"].MIN_AGE && x.age <  CREDIT_PARAMETERS["PROV_PROL_CARRIERE"].MAX_AGE && x.revenue_type.emploi;
    }
);
...
cp27.reference = "http://cffp.recherche.usherbrooke.ca/faq-items/credit-dimpot-pour-la-prolongation-de-carriere/";

let cp28 = Credit("Crédit d'impôt pour la prolongation de carrière",
    (x) => {
        return x.revenue > CREDIT_PARAMETERS["PROV_PROL_CARRIERE"].MIN_REV && x.age >= 65 && x.revenue_type.emploi
    }
);
...
Depuis 2016, tout revenu provenant d’un emploi auprès d’un employeur avec lequel le particulier a un lien de dépendance n’est pas admissible au crédit. Ainsi, le salaire d’un actionnaire-employé
(par exemple, un professionnel incorporé) reçu de la société dont il est actionnaire pourrait ne pas donner droit au crédit.`;
cp28.reference = "http://cffp.recherche.usherbrooke.ca/faq-items/credit-dimpot-pour-la-prolongation-de-carriere/";


let cp29 = Credit("Crédit d'impôt pour cotisations syndicales et professionnelles",
    (x) => {
...
cp29.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/cotisation-syndicale-ou-professionnelle/#sources";




let cp31 = Credit("Crédit pour maintien à domicile des aînés",
    (x) => {
        return x.age >= CREDIT_PARAMETERS["PROV_MAINTIEN_DOM_AINES"].MIN_AGE
            && (!x.hasPartner || (x.hasPartner && x.partner.age < CREDIT_PARAMETERS["PROV_MAINTIEN_DOM_AINES"].MAX_PARTNER_AGE))
            && revenu_familial(x) < CREDIT_PARAMETERS["PROV_MAINTIEN_DOM_AINES"].MAX_REV_FAM
            && x.depenses_maintien_domicile == "true";
    }
);
cp31.category = "Mesures relatives aux aînés";
cp31.type_credit = "Remboursable";
cp31.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-maintien-domicile-aines/";

let cp32 = Credit("Crédit pour maintien à domicile des aînés",
    (x) => {
        return x.age >= CREDIT_PARAMETERS["PROV_MAINTIEN_DOM_AINES_2"].MIN_AGE
            && x.hasPartner
            && x.partner.age >= CREDIT_PARAMETERS["PROV_MAINTIEN_DOM_AINES_2"].MIN_PARTNER_AGE
            && revenu_familial(x) < CREDIT_PARAMETERS["PROV_MAINTIEN_DOM_AINES_2"].MAX_REV_FAM
            && x.depenses_maintien_domicile == "true";
    }
);
cp32.category = "Mesures relatives aux aînés";
cp32.type_credit = "Remboursable";
cp32.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-maintien-domicile-aines/";

//----------------------------------------------------------------------------------------------------------------
// MESURES FEDERALES //// MESURES FEDERALES //// MESURES FEDERALES //// MESURES FEDERALES //// MESURES FEDERALES //
//----------------------------------------------------------------------------------------------------------------

let cf1 = Credit("Montant pour revenu de pension",
    (x) => {
        return x.revenue_type.retraite;
    }
);
cf1.category = "Mesures relatives aux aînés";
cf1["note"] = `Pour le montant pour revenu de pension (Fédéral), si une partie du crédit est inutilisée, elle peut être transférée à l’époux ou au conjoint de fait. Les revenus de pension admissibles au crédit
d’impôt se limitent à certains types de revenus et varient en fonction de l’âge du particulier.`;
cf1["reference"] = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/montant-revenu-pension-montant-revenus-retraite/";
cf1.type_credit = "Non remboursable";

let cf2 = Credit("Crédit d'impôt pour accessibilité domiciliaire",
    (x) => {
        return (x.age >= CREDIT_PARAMETERS["FED_ACCES_DOMICILIAIRE"].MIN_AGE
                || (x.hasPartner && x.partner.age >= CREDIT_PARAMETERS["FED_ACCES_DOMICILIAIRE"].MIN_PARTNER_AGE)) 
                && (x.depenses_renovation_admissibles == "true" || x.sante.depenses_renovations_admissibles == "true");
    }
);
cf2.category = "Mesures relatives à la santé";
cf2.type_credit = "Non remboursable";
cf2["note"] = `Le crédit d’impôt remboursable pour le soutien des aînés est partageable entre les conjoints selon la proportion dont ils auront convenu au moyen d’un formulaire prescrit TP-1029.SA
« Crédit d’impôt pour soutien aux aînés ». À défaut d’entente entre les particuliers, le ministre du Revenu déterminera le montant du crédit d’impôt que chacun pourra demander.

<br>
ARC,Ligne 398 – Dépenses pour l’accessibilité domiciliaire.`;
cf2["reference"] = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-impot-accessibilite-domiciliaire/";


let cf3 = Credit("Fractionnement du revenu de pension",
    (x) => {
...
[2] Par. 160(1.3) LIR et art. 1034.0.0.3 LI.
`;
cf3.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/fractionnement-revenus-pension-et-fractionnement-revenus-retraite-conjoints/";
...
cf6.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-epoux-ou-conjoint-de-fait/";

let csf1 = Credit("Crédit d'impôt pour frais médicaux",
    (x) => {
...
csf1.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-impot-frais-medicaux/";
csf1.type_credit = "Non remboursable";

let csf2 = Credit("Crédit d'impôt pour personnes handicapées",
    (x) => {
...
csf2.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-impot-personnes-handicapees-credit-impot-deficience-grave-prolongee/";

let csf4 = Credit("Supplément remboursable pour frais médicaux",
    (x, app_parameters) => {
        let params = app_parameters["FED_SUPPL_FRAIS_MED"];
        return revenu_familial(x) < params.MAX_REV_FAM 
...
csf4.reference = "https://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/supplement-et-credit-impot-frais-medicaux/";

let csf5 = Credit("Crédit canadien pour aidant naturel",
    (x) => {
        return x.sante.aidant_naturel == 'true';
    }
)
csf5.lien_additionnel = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-canadien-aidant-naturel/credit-canadien-pour-aidant-naturel-simulateur/#personne_charge_majeure"
...
[2] Al. 118(4)c) LIR.`
csf5.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-canadien-aidant-naturel/";

let csp3 = Credit("Crédit d'impôt pour aidant naturel d'une personne majeure",
    (x) => {
        return x.sante.prend_soin_conjoint == 'true';
    }
)
csp3.type_credit = "Remboursable";
csp3.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-impot-aidant-naturel-personne-majeure/";
csp3.lien_additionnel = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-canadien-aidant-naturel/credit-canadien-pour-aidant-naturel-simulateur/#personne_charge_majeure"
...
let csp1 = Credit("Crédit d'impôt pour frais médicaux",
    (x) => {
        return x.sante.frais_medicaux_admissibles == 'true';
    }
)
...
<br><br>
[1] Notons que les frais payés pour des personnes majeures à charge sont aussi à considérer dans le total des frais médicaux au Québec.`;
csp1.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-impot-frais-medicaux/";

let csp2 = Credit("Crédit d'impôt pour déficience grave et prolongée",
    (x) => {
        return x.sante.deficience_grave_prolongee == 'true';
    }
)
...
<br>
[2] ARC,Crédit d’impôt pour personnes handicapées.`;
csp2.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-impot-personnes-handicapees-credit-impot-deficience-grave-prolongee/";


let csp4 = Credit("Crédit d'impôt pour aidant naturel d'une personne majeure",
    (x) => {
        return x.sante.heberge_proche_admissible == 'true';
    }
)
csp4.type_credit = "Remboursable";
csp4.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-impot-aidant-naturel-personne-majeure/";
...
csp4.lien_additionnel = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-canadien-aidant-naturel/credit-canadien-pour-aidant-naturel-simulateur/#personne_charge_majeure";


let csp5 = Credit("Crédit d'impôt pour aidant naturel d'une personne majeure",
    (x) => {
        return x.sante.cohabite_proche_deficient == 'true';
    }
)
csp5.lien_additionnel = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-canadien-aidant-naturel/credit-canadien-pour-aidant-naturel-simulateur/#personne_charge_majeure"
csp5.type_credit = "Remboursable";
csp5.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-impot-aidant-naturel-personne-majeure/";
...
let csp6 = Credit("Crédit d'impôt pour aidant naturel d'une personne majeure",
    (x) => {
        return x.sante.aide_personne_sans_cohabiter == 'true';
    }
)
csp6.type_credit = "Remboursable";
csp6.lien_additionnel = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-canadien-aidant-naturel/credit-canadien-pour-aidant-naturel-simulateur/#personne_charge_majeure"
csp6.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-impot-aidant-naturel-personne-majeure/";
...
let csp7 = Credit("Crédit d'impôt pour relève bénévole",
    (x) => {
        return x.sante.heures200 == 'true' && x.sante.fournis_service_releve_benevole;
    }
)
csp7.type_credit = "Remboursable";
csp7.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-impot-releve-benevole/";

let csp9 = Credit("Crédit d'impôt pour aidant naturel d'une personne majeure",
    (x) => {
        return x.sante.aidant_naturel == 'true';
    }
)
csp9.type_credit = "Remboursable";
csp9.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-canadien-aidant-naturel/";
csp9.lien_additionnel = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-canadien-aidant-naturel/credit-canadien-pour-aidant-naturel-simulateur/#personne_charge_majeure"
...
let cep1 = Credit("Déduction pour bourse d'études",
    (x) => {
...
cep1.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/exemption-bourse-etudes/";
cep1.type_credit = "Déduction";


let cef1 = Credit("Exemption pour bourse d'études",
    (x) => {
        return x.etude.recu_bourse == 'true';
    }
);
...
<br>
[2] ARC, Folio de l’impôt sur le revenu S1-F2-C3, « Bourses d’études, subventions de recherches et autres montants d’aide à l’éducation » (26 avril 2017), par. 3.14.`;
cef1.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/exemption-bourse-etudes/";
cef1.type_credit = "Exemption";
cef1.type = "Fédéral";

let cepf2 = Credit("Crédit d'impôt pour intérêts payés sur un prêt étudiant",
    (x) => {
...
cepf2.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-impot-interets-payes-pret-etudiant/";
cepf2.type_credit = "Non remboursable";
cepf2.type = "Fédéral & Provincial";
cepf2.category = "Mesures relatives aux études";

let cdpf1 = Credit("Crédit d'impôt pour contributions politiques",
    (x) => {
        return x.dons.contribution_verse == 'true';
    }
);
cdpf1.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-impot-contributions-politiques/";
cdpf1.type_credit = "Non remboursable";


let cdpf2 = Credit("Crédit d'impôt pour dons",
    (x) => {
...
admissible au don. 
`;
cdpf2.reference = " http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-impot-dons/";
cdpf2.type_credit = "Non remboursable, reportable";


let cp30 = Credit("Crédit d'impôt pour séjour dans une unité transitoire de récupération fonctionnelle",
    (x) => {
...
cp30.reference = " http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-impot-frais-engages-aine-maintenir-autonomie/";


let cp12 = Credit("Allocation famille", //REMBOURSABLE
    (x) => {
        return hasKid(x, kid => {
            return kid.age < 18
        });
...
`;
cp12.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/allocation-famille-soutien-enfants/";


let cp8 = Credit("Crédit d'impôt pour frais engagés par un aîné pour maintenir son autonomie",
    (x) => {
        return x.age >= 70 && x.biens_admissibles;
    }
);
cp8.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-impot-frais-engages-aine-maintenir-autonomie/"
cp8.type_credit = "Remboursable";
cp8.category = "Mesures relatives aux aînés";


let cp10 = Credit("Crédit d'impôt pour frais de garde d'enfants", //REMBOURSABLE
    (x) => {
        return hasKid(x, (kid) => {
            return kid.age <= 16  //Not in suggested parameter sheet?
    ...
    (x, cp) => {
        ...
        if (x.hasKids) {
            ...
            if(x.revenue > params.MAX_REV){
                maxkidrev = maxkidrev - ((x.revenue - params.MIN_REV_P2)*0.01457);
            }
            if(x.revenue >= params.MIN_REV_P2){
...
cf7.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-pour-personne-a-charge-admissible/";

let cf16 = Credit("Déduction pour frais de garde",
    (x) => {
        return hasKid(x, (kid) => {
            return kid.age < 16 
            && kid.frais_engages.frais_garde == true
...
cf16.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/deduction-frais-garde-enfants/";
cf16.type_credit = "Déduction";

let cf15 = Credit("Déduction fédérale pour cotisations syndicales et professionnelles",
    (x) => {
        return x.frais_engages.cotisation_syndicale_professionnelle;
    }
);
...
cf15.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/cotisation-syndicale-ou-professionnelle/";

let cf14 = Credit("Déduction pour frais de déménagement",
    (x) => {
        return x.frais_engages.demenagement;
    }
);
...
de la fréquentation d'un établissement d'enseignement. Le lien requis n'existe pas si un particulier déménage uniquement pour des raisons personnelles.`;
cf14.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/frais-demenagement/";
...
let csp8 = Credit("Crédit d'impôt remboursable pour frais médicaux", //TODO: INCORRECT VALUES?
    (x) => {
        return x.sante.frais_medicaux_admissibles == 'true' 
        && (x.revenue_type.emploi || x.revenue_type.travailleur_autonome) 
        && revenu_familial(x) < 47400 && x.revenue >= 3080;
    },
)
csp8.type_credit = "Remboursable";
csp8.reference = "https://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/supplement-et-credit-impot-frais-medicaux/";

let cf8 = Credit("Crédit d'impôt pour la TPS ",
    (x, cp) => {
...
cf8.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-impot-tps-tvh/";

let cf5 = Credit("Allocation canadienne pour enfants", //Prestation 
    (x, parameters) => {
        let cp = parameters["FED_ALLOC_ENFANT"];
        let mat_admiss = [
            [null, cp._1BELOW6_0ABOVE6_MAXREV, cp._2BELOW6_0ABOVE6_MAXREV, cp._3BELOW6_0ABOVE6_MAXREV, cp._4BELOW6_0ABOVE6_MAXREV], // 0 enfants 6 a 18
            [cp._0BELOW6_1ABOVE6_MAXREV, cp._1BELOW6_1ABOVE6_MAXREV, cp._2BELOW6_1ABOVE6_MAXREV, cp._3BELOW6_1ABOVE6_MAXREV],
            [cp._0BELOW6_2ABOVE6_MAXREV, cp._1BELOW6_2ABOVE6_MAXREV, cp._2BELOW6_2ABOVE6_MAXREV],
        ...
        return x.hasKids && revenu_familial(x) < mat_admiss[six_to_eighteen][under_six];
    }
);
...
`;
cf5.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/allocation-canadienne-enfants/";


let cp16 = Credit("Crédit d'impôt attribuant une prime au travail",
    (x, parameters) => {
...
cp16.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-impot-remboursable-prime-travail/";
...
let cp17 = Credit("Crédit d'impôt attribuant une prime au travail",
    (x, parameters) => {
        let cp = parameters["PROV_PRIM_TRAVAIL"];
        return (x.revenue_type.emploi || x.revenue_type.travailleur_autonome)
            && !x.hasPartner
            && (hasEnfantsAuxEtudesTempsPlein(x) || hasKid(x, (kid) => {
                return kid.age < 18
            }))
...
let cp18 = Credit("Crédit d'impôt attribuant une prime au travail",
    (x, params) => {
...
let cp19 = Credit("Crédit d'impôt attribuant une prime au travail",
    (x, params) => {
        let cp = params["PROV_PRIM_TRAVAIL"];
        return (x.revenue_type.emploi || x.revenue_type.travailleur_autonome)
            && x.hasPartner
...
let cp14 = Credit("Crédit d'impôt pour solidarité", //REMBOURSABLE
    (x, params) => {
        ...
        if (x.logement_admissible == 'true') {
            let seuils_celib = [cp.NP_L_0K_MAXREV, cp.NP_L_1K_MAXREV, cp.NP_L_2K_MAXREV, cp.NP_L_3K_MAXREV, cp.NP_L_4K_MAXREV];
            let seuils_couple = [cp.P_L_0K_MAXREV, cp.P_L_1K_MAXREV, cp.P_L_2K_MAXREV, cp.P_L_3K_MAXREV, cp.P_L_4K_MAXREV];
            ...
            if (!x.hasPartner && x.revenue < cp.NP_NL_M_MAXREV && (kids_over_18 > 0 || x.plusieurs_props_locs == "true")) {
                return true;
            }
            if (!x.hasPartner && x.revenue < cp.NP_NL_NM_MAXREV && (kids_over_18 == 0 && x.plusieurs_props_locs == "false")) {
...
cp14.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-impot-solidarite/";

let cf10 = Credit("Allocation canadienne pour les travailleurs",
    (x, params) => {
...
cf10.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/allocation-canadienne-pour-les-travailleurs/";


let cf11 = Credit("Allocation canadienne pour les travailleurs",
    (x, params) => {
        let cp = params["FED_ALLOC_TRAVAILLEURS"];
        return (x.revenue_type.emploi || x.revenue_type.travailleur_autonome)
            && !x.hasPartner
...
cf11.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/allocation-canadienne-pour-les-travailleurs/";


let cf12 = Credit("Allocation canadienne pour les travailleurs",
    (x, params) => {
        let cp = params["FED_ALLOC_TRAVAILLEURS"];
        return (x.revenue_type.emploi || x.revenue_type.travailleur_autonome)
            && x.hasPartner
...
cf12.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/allocation-canadienne-pour-les-travailleurs/";



let cf13 = Credit("Allocation canadienne pour les travailleurs",
    (x, params) => {
        let cp = params["FED_ALLOC_TRAVAILLEURS"];
        return (x.revenue_type.emploi || x.revenue_type.travailleur_autonome)
            && x.hasPartner
...
cf13.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/allocation-canadienne-pour-les-travailleurs/";


let cp0_a = Credit("Crédit d'impôt pour personne vivant seule",
    (x, params) => {
...
let cp0_b = Credit("Crédit d'impôt pour personne vivant seule",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
        return x.age < 65
            && !x.hasPartner
            ...
            && !hasKid(x, (kid) => {
                return kid.age >= 18 && (kid.status_etudiant == "temps_partiel" || kid.status_etudiant == "pas_aux_etudes")
            })
            && !(x.plusieurs_props_locs == "true")
            && !x.revenue_type.retraite
...
let cp1_2 = Credit("Crédit en raison de l'âge",// (Crédit en raison de l'âge seulement)
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
...
let cp1_3 = Credit("Crédit en raison de l'âge",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
        return x.age >= 65
            && (x.hasPartner && x.partner.age >= 65)
...
let cp1_4 = Credit("Crédit en raison de l'âge",
    (x) => {
        let kidO18NotStudyFullTime = hasKid(x, (kid) => {
            return kid.age >= 18 && kid.status_etudiant != "temps_plein";
        });
...
let cp1_4a = Credit("Montant pour revenus de retraite",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
        let kidO18NotStudyFullTime = hasKid(x, (kid) => {
            return kid.age >= 18 && kid.status_etudiant != "temps_plein";
...
let cp1_5 = Credit("Crédit en raison de l'âge",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
        return x.age >= 65
            && (x.hasPartner && x.partner.age >= 65)
...
let cp1_5a = Credit("Montant pour revenus de retraite",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
        return x.age >= 65
            && (x.hasPartner && x.partner.age >= 65)
...
let cp1_6 = Credit("Crédit en raison de l'âge",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
        return x.age >= 65
            && !x.hasPartner
...
let cp1_6a = Credit("Crédit d'impôt pour personne vivant seule",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
        return x.age >= 65
            && !x.hasPartner
...
let cp1_7 = Credit("Crédit en raison de l'âge",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
...
let cp1_7a = Credit("Crédit d'impôt pour personne vivant seule",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
        let kidO18StudyFullTime = hasKid(x, (kid) => {
            return kid.age >= 18 && kid.status_etudiant == "temps_plein";
...
let cp1_8 = Credit("Crédit en raison de l'âge",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
...
let cp1_8a = Credit("Crédit d'impôt pour personne vivant seule",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
...
let cp1_8b = Credit("Montant pour revenus de retraite",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
...
let cp1_9 = Credit("Crédit en raison de l'âge",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
        let kidO18StudyFullTime = hasKid(x, (kid) => {
            return kid.age >= 18 && kid.status_etudiant == "temps_plein";
...
let cp1_9a = Credit("Crédit d'impôt pour personne vivant seule",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
        let kidO18StudyFullTime = hasKid(x, (kid) => {
            return kid.age >= 18 && kid.status_etudiant == "temps_plein";
...
let cp1_9b = Credit("Montant pour revenus de retraite",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
        let kidO18StudyFullTime = hasKid(x, (kid) => {
            return kid.age >= 18 && kid.status_etudiant == "temps_plein";
...
function addMontantRetraiteInfo(cred) {
    ...
    particuliers.`;
    cred.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/montant-revenu-pension-montant-revenus-retraite/";
    cred.category = "Mesures relatives aux aînés";
    cred.type = "Provincial";
    cred.type_credit = "Non remboursable";
}

let cpf2_1 = Credit("Montant pour revenus de retraite",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
        let kidO18NotStudyFullTime = hasKid(x, (kid) => {
            return kid.age >= 18 && kid.status_etudiant != "temps_plein";
...
let cpf2_2 = Credit("Montant pour revenus de retraite",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
        let kidO18NotStudyFullTime = hasKid(x, (kid) => {
            return kid.age >= 18 && kid.status_etudiant != "temps_plein";
...
let cpf2_2a = Credit("Crédit en raison de l'âge",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
        let kidO18NotStudyFullTime = hasKid(x, (kid) => {
            return kid.age >= 18 && kid.status_etudiant != "temps_plein";
...
let cpf2_3 = Credit("Montant pour revenus de retraite",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
        return x.age >= 65
            && (x.hasPartner && x.partner.age >= 65)
...
let cpf2_3a = Credit("Crédit en raison de l'âge",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
        return x.age >= 65
            && (x.hasPartner && x.partner.age >= 65)
...
let cpf2_4 = Credit("Montant pour revenus de retraite",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
...
let cpf2_4a = Credit("Crédit d'impôt pour personne vivant seule",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
...
let cpf2_5 = Credit("Montant pour revenus de retraite",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
        let kidO18NotStudyFullTime = hasKid(x, (kid) => {
            return kid.age >= 18 && kid.status_etudiant == "temps_plein";
...
let cpf2_5a = Credit("Crédit d'impôt pour personne vivant seule",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
        let kidO18StudyFullTime = hasKid(x, (kid) => {
            return kid.age >= 18 && kid.status_etudiant == "temps_plein";
...
let cpf2_6 = Credit("Montant pour revenus de retraite",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
        let kidO18NotStudyFullTime = hasKid(x, (kid) => {
            return kid.age >= 18 && kid.status_etudiant != "temps_plein";
...
let cpf2_6a = Credit("Crédit en raison de l'âge",
    (x) => {
        let kidO18NotStudyFullTime = hasKid(x, (kid) => {
            return kid.age >= 18 && kid.status_etudiant != "temps_plein";
        });
...
let cpf2_6b = Credit("Crédit d'impôt pour personne vivant seule",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
        let kidO18StudyFullTime = hasKid(x, (kid) => {
            return kid.age >= 18 && kid.status_etudiant != "temps_plein";
...
let cpf2_7 = Credit("Montant pour revenus de retraite",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
        let kidO18StudyFullTime = hasKid(x, (kid) => {
            return kid.age >= 18 && kid.status_etudiant == "temps_plein";
...
let cpf2_7a = Credit("Crédit en raison de l'âge",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
        let kidO18StudyFullTime = hasKid(x, (kid) => {
            return kid.age >= 18 && kid.status_etudiant == "temps_plein";
...
let cpf2_7b = Credit("Crédit d'impôt pour personne vivant seule",
    (x, params) => {
        let cp = params["AGE_RET_PVS"];
        let kidO18StudyFullTime = hasKid(x, (kid) => {
            return kid.age >= 18 && kid.status_etudiant == "temps_plein";
...
function Credit(name, eligibility_predicate, montant_credit, seuil_reduction, taux_reduction, montant_base = 0, revenu_admissible = null) {
    let cr = {
        "nom": name,
...
let prov = [cp4, cp5, cp6, cp7, cp8, cp9, cp10, cp11, cp12, cp13, cp14, cp15, cp16, cp17, cp18, cp19, cp20, cp21, cp25, cp26, cp27, cp28, cp29, cp30, cp31, cp32];
fed.push(cf1);
fed.push(cf2);
fed.push(cf3);
fed.push(cf5);
...
function hasEnfantsOver18AuxEtudesTempsPlein(userProfile) {
    if (!userProfile.hasKids) return false;
    for (let i = 0; i < userProfile.kidAmount; i++) {
        if (userProfile.kids[i].status_etudiant == "temps_plein" && userProfile.kids[i].age >= 18) return true;
    }
    return false;
}
...
function hasKid(x, pred) { //TODO: use this function everywhere possible
    if (!x.hasKids) {
        return false;
    } else {
        for (let i = 0; i < x.kidAmount; i++) {
...
function addCreditAgeInfo(cred) {
    cred.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-impot-age/";
    cred.type_credit = "Non remboursable";
    cred.category = "Mesures relatives aux aînés";
    cred.type = "Provincial";
...
function addVivantSeuleInfo(cred) {
    cred.type_credit = "Non remboursable";
    cred.reference = "http://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/credit-personne-vivant-seule/";
    cred.type = "Provincial"
...
let arr = [cp0_a, cp0_b, cp1_2, cp1_3, cp1_4, cp1_4a, cp1_5, cp1_5a, cp1_6, cp1_6a, cp1_7, cp1_7a, cp1_8, cp1_8a, cp1_8b, cp1_9, cp1_9a, cpf2_1, cpf2_2, cpf2_2a, cpf2_3, cpf2_3a, cpf2_4, cpf2_4a, cpf2_5, cpf2_5a, cpf2_6, cpf2_6a, cpf2_6b, cpf2_7, cpf2_7a];
